export const parseJson = ( data, setNodes, setEdges, setError ) => {
    ...
    if(jsonObject == null || !Array.isArray(jsonObject) || jsonObject.length === 0) {
        setNodes(null);
        setEdges(null);
        ...
        for (const key in item.connections) {
            ...
            for (const edge of connection.edges) {
                ...
                if (parentNode && currentNode) {
                    const sourceNode = nodes.find((node) => node.data.label === parentNode);
                    const targetNode = nodes.find((node) => node.data.label === currentNode);
                    ...
                    const targetNodeId = targetNode ? targetNode.id : currentNode;
                    ...
                    if (sourceNodeId && targetNodeId) {
                        ...
                        console.error(`Skipping edge creation for nodes that don't exist: source '${parentNode}' and target '${currentNode}'`);
                    }
                }
            }
        }
